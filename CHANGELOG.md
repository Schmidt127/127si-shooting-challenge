...
#### Changed
- **Email producer boolean input parsing (2026-09-14)** — Replaces `Boolean(cfg.testMode)` (and similar) so Airtable text `"false"` is false. Shared `parseAutomationBoolean` / `parseAutomationSendMode` inlined into **071 v4.6**, **072 v4.9.3**, **073 v4.10**, **074 v3.7**, **076 v8.16**, **078A v1.8**, **117 v2.3**, **118 v2.2**, **119 v1.9**. Triggers and recipient routing unchanged. Checklist: [`docs/deploy-checklists/EMAIL-PRODUCER-BOOLEAN-INPUT-PARSING.md`](./docs/deploy-checklists/EMAIL-PRODUCER-BOOLEAN-INPUT-PARSING.md). **Production paste pending. No simulation executed.**
