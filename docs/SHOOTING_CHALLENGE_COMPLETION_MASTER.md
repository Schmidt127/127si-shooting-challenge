...
Execution matrix IDs advanced: PKG-045 — repository-only active-XP Source Key
integrity gate (2026-09-14): shared validator for blank/whitespace keys, exact
duplicates, and HOMEWORK_XP| + HOMEWORK_COMPLETION| dual-key; wired into business
reconciliation and read-only reconciliation checker. No Airtable writes; no
simulation execute.
...
| `PKG-045` | Perfect-season XP oracle fixture for Athlete 1 post-run comparison | `repository-ready` | `tools/season_simulation/expected_perfect_season_xp.json`; SC-SEASON-SIM-002; `tools/season_simulation/active_xp_source_key_integrity.py` | Live Production XP Reward Rules (`appn84sqPw03zEbTT`); season-sim calendar Apr 25–Jun 30 2027 (67 days) | Cursor; Mike | Independent expected Lifetime XP fixture; source-by-source and chronological totals match; simulation engine does not read the fixture to award XP; active-XP Source Key integrity enforced on reconciliation | 2026-09-14 — Execution matrix IDs advanced: PKG-045 — shared active-XP Source Key integrity validator + Stage Z formula lifecycle (PR #540); oracle **4980** path; no Airtable writes; no simulation execute | Repo | Use after SC-SEASON-SIM execute to compare Enrollment Lifetime XP | 45 |

Execution matrix IDs advanced: PKG-045 — perfect-season XP oracle fixture (4910 XP / 67-day window; Zoom 90) for Athlete 1 post-run comparison only.
