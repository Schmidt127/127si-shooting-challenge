...
| **078A** | Enrollment — Create WELCOME Email Handoff | Enrollments after Athlete + cleaned parent email + Program Instance | `078A-email-notifications-and-external-handoffs-enrollment-create-welcome-email-handoff.js` (**v1.6** — queue create only; optional `testMode` input default true; `WELCOME\|ENROLLMENTS\|{Enrollment Id}`; does **not** write retired 075 Enrollment fields) |
