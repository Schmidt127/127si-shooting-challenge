...
Last GitHub Update: 2026-09-13 (v10.11 SC-SEASON-SIM-001-DEPLOY-20260913B)
 ...
 *
 * VERSION HISTORY
 * - v10.11 (2026-09-13): SC-SEASON-SIM-001-DEPLOY-20260913B — version bump for
 *   Airtable draft verification only. Soft-skip behavior unchanged from v10.10.
 * - v10.10 (2026-09-13): Soft-skip (do not throw / do not ack) when
 *   Total Homework XP Awarded is not yet positive so premature recordMatchesConditions
 *   runs against Needed=1 before 064 settles Base XP cannot latch the HC forever.
 *   Production Perfect sim: 065 v10.8 failed at "Total Homework XP Awarded must be
 *   positive" then Needed stayed 1 after 064 wrote Base XP=35 (no 0→1 re-entry).
 *   REQUIRED trigger (UI): Needed?=1 AND Total Homework XP Awarded > 0.
...
async function main() {
  ...
  if (!(totalXp > 0)) {
    // Soft-skip: do not ack signature and do not throw. When 064 later sets Base XP,
    // Total XP becomes positive; with trigger Needed=1 AND Total XP > 0, 065 re-enters.
    // Throwing here historically latched Needed=1 forever (recordMatchesConditions).
    return finish(CONFIG.statuses.skipped, "skipped_total_xp_not_ready", {
      step: debugStep,
      key,
      totalXp: totalXp == null ? "" : totalXp,
      note: "Total Homework XP Awarded not positive yet; waiting for 064 / formula settle.",
    });
  }
