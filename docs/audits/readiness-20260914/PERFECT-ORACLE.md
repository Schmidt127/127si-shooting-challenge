# Perfect Mike Schmidt Season Oracle

**Run ID (planned / not executed):** SEASON-SIM-PERFECT-20260914T131218Z-mike-schmidt  
**Status:** Prepared oracle only — **simulation NOT EXECUTED** (any interrupted attempt was cleaned; transactional bases returned to zero-state).  
**Published:** 2026-09-14T13:12:18.501342+00:00  
**Window:** 2027-04-25 → 2027-06-30 (67 days)  
**Grade band:** 9-12 / season goal 12,000  
**PHA:** 20 (Early Bird + Weeks 1–9 × 2)  
**Parent Email:** schmidt@fairfieldbasketballclub.com  

| Bucket | Expected event count | Expected active XP |
| --- | ---: | ---: |
| Submission Base | 67 | 1340 |
| Homework | 20 | 700 |
| Video Feedback | 30 | 750 |
| Streak | 9 | 455 |
| Weekly Threshold | 26 | 480 |
| Perfect Week | 10 | 1000 |
| Shot Milestone | 6 | 165 |
| Zoom | 2 attendance records | 90 |
| Other active bucket | 0 | 0 |
| **Total** | | **4980** |

**Level:** G.O.A.T.  
**Pass condition:** Enrollment Lifetime XP Earned == sum(Active XP Points) == 4980  
**Source:** tools/season_simulation/expected_perfect_season_xp.json  
